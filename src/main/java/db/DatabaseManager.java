...
/** Singleton SQLite data layer; manages all tables with duplicate-safe INSERT OR IGNORE seeding including ASR-style training phrases. */
public class DatabaseManager {
    ...
    public void cancelAllAlarms() {
        String sql = "DELETE FROM alarms WHERE is_triggered = 0";
        try (PreparedStatement p = connection.prepareStatement(sql)) {
            p.executeUpdate();
        } catch (SQLException e) {
            System.err.println("[DB] cancelAllAlarms: " + e.getMessage());
        }
    }
