...
/** HTTP REST client for the Flask Python bridge (:5000); manages process lifecycle, health polling, and all API calls. */
public class PythonBridge {
    ...
    /** Kills any stale process on the given port (Windows only) to prevent connecting to a zombie Flask process. */
    ...
    /** Polls /health until 200; silently retries on ConnectException (not up yet) and 503 (still loading). */
    ...
    /** Long-polls /command; returns {text, intent} on 200 or null on 204 (no speech yet). */
    ...
    /** Tell Python to start the gesture controller and open the camera. */
    public void startGestures() throws Exception {
        postNoBody(BASE_URL + "/gesture/start");
    }

    /** Tell Python to stop the gesture controller and release the camera. */
    public void stopGestures() throws Exception {
        postNoBody(BASE_URL + "/gesture/stop");
    }
