...
/**
 * Frontend facade for the backend's db.DatabaseManager singleton.
 *
 * All calls delegate to db.DatabaseManager.getInstance() so both layers
 * share the same SQLite connection and data. This class provides the
 * com.rahbar.db package path that all frontend UI classes import, plus
 * the inner CommandLog and Alarm types the frontend expects.
 *
 * Backend code continues to use db.DatabaseManager directly — no changes needed there.
 */
