 ...
 * RegistryController
 *
 * Manages the App Registry and Website Registry tabs.
 *
 * Each tab shows a table loaded from SQLite.
 * The user can:
 *   - Add new entries (alias + path/URL)
 *   - Browse for an executable using a file picker (apps only)
 *   - Remove entries with a Delete button in each row
 *
 * All changes write directly to SQLite so they take effect for the
 * backend immediately — no restart needed.
 */
