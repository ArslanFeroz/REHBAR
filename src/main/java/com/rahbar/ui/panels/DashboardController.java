 ...
 *   - greetingLabel / greetingSubLabel driven by time-of-day + profile_nickname
 *   - activityChart (BarChart) populated with hourly command counts for today
 *
 * All original fx:id bindings (statusIcon, statusTitle, statCommandsToday, etc.)
 * are preserved exactly — no backend changes needed.
 */
public class DashboardController implements Initializable {
    ...
    /**
     * Builds the greeting string based on current hour:
     *   00-11 → Good morning
     *   12-16 → Good afternoon
     *   17-20 → Good evening
     *   21-23 → Good night
     *
     * Appends the user's nickname if saved in the profile.
     * Falls back to "R.A.H.B.A.R is ready." if no name is set.
     */
    private void buildGreeting() {
        ...
        String name = null;
        try {
            name = DatabaseManager.getInstance().getSetting("profile_nickname");
    ...
    /**
     * Populates the activity chart with hourly command counts.
     * Shows the 8 most recent hours (so chart isn't too wide).
     * Falls back to zeros if DB isn't ready yet.
     */
    private void populateChart() {
        ...
        try {
            // Try to get hourly data from DB
            // DatabaseManager.getCommandCountByHour() returns int[24] (index = hour)
            int[] hourlyCounts = DatabaseManager.getInstance().getCommandCountByHour();

            int currentHour = LocalTime.now().getHour();
            // Show the last 8 hours up to now
            for (int i = 7; i >= 0; i--) {
            ...
            // DB not ready or method not implemented yet — show placeholder zeros
            int currentHour = LocalTime.now().getHour();
            for (int i = 7; i >= 0; i--) {
                int h = (currentHour - i + 24) % 24;
                series.getData().add(
        ...
        // Style the bars to use the theme accent color via CSS
    ...
    // ─────────────────────────────────────────────────────────────────────────
    // Table setup (unchanged from original)
    // ─────────────────────────────────────────────────────────────────────────
    ...
    private void loadData() {
        Thread loader = new Thread(() -> {
            try {
                ...
                Platform.runLater(() -> {
                    buildGreeting(); // still show greeting even if DB fails
    ...
    // ─────────────────────────────────────────────────────────────────────────
    // Status banner polling (unchanged)
    // ─────────────────────────────────────────────────────────────────────────
    ...
    public void onHide() { polling = false; }

    // ─────────────────────────────────────────────────────────────────────────
    // CommandRow model (unchanged)
    // ─────────────────────────────────────────────────────────────────────────
