...
/**
 * SystemInfoService -- answers queries about time, date, battery, CPU, RAM, disk.
 *
 * Fixes vs original:
 *  - query() checks "battery" BEFORE "time" so "battery time remaining"
 *    is not accidentally routed to the time branch.
 *  - getBatteryInfo() runs ONE PowerShell command returning both charge level
 *    and charging status -- faster, atomic, no race between two processes.
 *  - getBatteryInfo() drains output stream BEFORE waitFor() to prevent
 *    OS pipe-buffer deadlock.
 *  - getBatteryInfo() uses waitFor(5, SECONDS) timeout so a hung PowerShell
 *    never blocks the command thread indefinitely.
 *  - getCpuUsage() restores interrupt flag on InterruptedException.
 *  - getDiskUsage() added (was returning "I cannot retrieve that information").
 */
