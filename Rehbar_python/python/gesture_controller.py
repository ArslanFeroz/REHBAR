...
GESTURE_LABELS = {
    ...
    'THUMBS_UP':       'Volume Up',
    'FIST':            'Volume Down',
    'POINT_1':         'Brightness Up',
    'POINT_2':         'Switch Tab',
    'POINT_3':         'Brightness Down',
    'POINT_4':         'Minimize Others',
    'ZOOM_IN':         'Zoom In',
    'ZOOM_OUT':        'Zoom Out',
...
class GestureController:
    ...
    def _classify(fingers) -> Optional[str]:
        ...
        # Pointing gestures — thumb folded, 1–4 non-thumb fingers extended upward
        if not t:
            if i and not m and not r and not p:        return 'POINT_1'
            if i and m and not r and not p:            return 'POINT_2'
            if i and m and r and not p:                return 'POINT_3'
            if i and m and r and p:                    return 'POINT_4'
        return None
    ...
    def _execute(self, gesture: str) -> None:
        ...
        try:
            ...
            elif gesture == 'POINT_1':      self._brightness_up()
            elif gesture == 'POINT_2':      self._switch_tab()
            elif gesture == 'POINT_3':      self._brightness_down()
            elif gesture == 'POINT_4':      self._minimize_others()
    ...
    def _brightness_up(self) -> None:
        try:
            ...
            val = cur[0] if isinstance(cur, list) else cur
            sbc.set_brightness(min(100, val + 10), display=0)
        except Exception as e:
            print(f'[Gesture] Brightness up failed: {e}')
    ...
    def _brightness_down(self) -> None:
        try:
            ...
            cur = sbc.get_brightness(display=0)
            val = cur[0] if isinstance(cur, list) else cur
            sbc.set_brightness(max(0, val - 10), display=0)
        except Exception as e:
            print(f'[Gesture] Brightness down failed: {e}')

    def _minimize_others(self) -> None:
        """Show desktop (Win+D) — minimizes all windows."""
        self._pyag.hotkey('win', 'd')
