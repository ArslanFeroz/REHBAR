...
class IntentClassifier:
    ...
    def predict_debug(self, text: str) -> Dict[str, Any]:
        ...
        if ai_enabled:
            if rule_intent in {
                ...
                'GESTURE_ENABLE', 'GESTURE_DISABLE',
                'CANCEL_ALARM'
            }:
                return {
                    'raw': raw,
    ...
    def _rule_intent(self, text: str) -> str | None:
        ...
        if re.search(r'\b(cancel|remove|delete|stop|clear|disable)\b.*\b(alarm|alarms|reminder|reminders|timer)\b', text):
            return 'CANCEL_ALARM'
        if re.search(r'\b(cancel|clear)\s+all\b', text):
            return 'CANCEL_ALARM'
        ...
        if re.search(r'\b(enable|turn on|start|activate)\b.*\b(gesture|gestures|camera)\b', text):
            return 'GESTURE_ENABLE'
        if re.search(r'\b(disable|turn off|stop|deactivate)\b.*\b(gesture|gestures|camera)\b', text):
            return 'GESTURE_DISABLE'
    ...
    def _load_db(self) -> Tuple[List[str], List[str]]:
        ...
        try:
            ...
            cur.execute("""
                        ...
                        FROM training_data
                        WHERE intent NOT IN ('UNKNOWN', 'CHAT', 'AI_ENABLE', 'AI_DISABLE', 'GESTURE_ENABLE', 'GESTURE_DISABLE')
                        """)
            rows = cur.fetchall()
