...
def voice_capture_loop(listener: VoiceListener,
    ...
    while True:
        ...
        try:
            ...
            if intent == 'GESTURE_ENABLE':
                if _gesture_ctrl is not None and not _gesture_ctrl.is_running():
                    _gesture_ctrl.start()
                    msg = 'Gesture control enabled. Camera is now active.'
                else:
                    msg = 'Gesture control is already running.'
                try:
            ...
            if intent == 'GESTURE_DISABLE':
                if _gesture_ctrl is not None and _gesture_ctrl.is_running():
                    _gesture_ctrl.stop()
                    msg = 'Gesture control disabled. Camera turned off.'
                else:
                    msg = 'Gesture control is not running.'
                try:
                    tts_queue.put_nowait(msg)
                except queue.Full:
                    pass
...
def main():
    ...
    print('[Bridge] Initialising gesture controller (camera off until enabled)...')
    ...
    # Camera is NOT started here — user must enable via settings or voice command.
    ...
    # Welcome message — uses the user-configured assistant name from the DB
    _assistant_name = _db_get_setting('assistant_name', 'RAHBAR')
    _WELCOME = (
        f"{_assistant_name} online. All systems operational. "
        "Ready when you are."
    )
    try:
