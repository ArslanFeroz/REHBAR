"""
massive_intent_classifier_test.py

A tougher evaluation harness for Rehbar's IntentClassifierV2.

What it tests:
1. Clean, hand-written commands
2. Ambiguous phrasing that can confuse intent boundaries
3. Voice-ASR style noise / misspellings
4. Wake-word + filler variants
5. OOD / unsupported requests that should become CHAT or UNKNOWN
6. Batch summary by expected label and route
7. Optional confusion matrix and CSV export

Usage:
    python massive_intent_classifier_test.py
    python massive_intent_classifier_test.py --csv results.csv
    python massive_intent_classifier_test.py --json results.json

Adjust the import below if your classifier module name/path is different.
"""
